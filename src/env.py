...
def two_to_one(two_table): #what means??
...
def get_minimum():
    ...
    for data_num in range(TXT_NUM):
        ...
        file_path = folder + LOCATION + "/" + file_name
        f = open(file_path, "r")
        f1 = f.readlines()
        # get line_num
        line_num = 0
...
class UE():
    def __init__(self, user_id, data_num):
        ...
        # write data to self.mob, update every 30 secs
        # in database, the data structure is col_0 = time, col 1 = x, col_2 = y
        now_sec = 0
        for line in f1:
            for sec in range(30):
        ...
        #generate request input 2 = output 1, finish 1 --> start 
...
class Env():
    ...
    def ddpg_step_forward(self, a, r_dim, b_dim):
        ...
        # rewards
        self.rewards = self.fin_req_count - self.prev_count # reward = number of tasks processed = task final - task previous
