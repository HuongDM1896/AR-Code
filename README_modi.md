...
+ State
  ...
  ```python
    def generate_state(two_table, U, E, x_min, y_min):
        ...
        count = 0
        for edge in E: #computing resource avai
            S[count] = edge.capability/(r_bound*10)
            count += 1
        for i in range(len(one_table)): #bandwidth resource avai
            S[count] = one_table[i]/(b_bound*10)
            count += 1
        for user in U: #user request to edge node id check, 1/0 = offload/no
            S[count] = user.req.edge_id/100
            count += 1
        for user in U: #location of user 
